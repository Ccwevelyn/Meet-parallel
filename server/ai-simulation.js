...
async function generateWithLLM(member, recentMessages, personas, options = {}) {
  ...
  const systemParts = [
    ...
    '【必守】当对方说「回复1」「扣1」「看到的回复1」「能看到的回复1」等（在要大家确认看到消息）时，你的回复必须是对「看到/收到」的确认，用该角色口吻说 1、看到了、在呢、1 收到、来了 等，不能答非所问（例如对方在要确认，你却只回「困」「在写作业」等与确认无关的内容）。可以带一点语气，但主题必须是「我看到了/收到了」。'
  ];
  ...
  if (recent) {
    ...
    if (options.replyToHuman) {
      userContent += `上一条是真人（群友）发的。请以「${displayName}」的身份、严格基于该角色人设对其做出回应或接话。若对方在要求「回复1」「扣1」「看到的回复1」等确认看到消息，你必须回复对「看到/收到」的确认（如 1、看到了、在呢、1 收到），不要答非所问（例如只回「困」）；其他情况用该角色会用的词和语气贴合话题。只输出这一条。`;
    } else {
